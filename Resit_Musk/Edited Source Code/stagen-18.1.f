...
C**********************************************************************************
C
      WRITE(8,*) ' POINTS ON THE BLADE TO BE USED FOR PLOTTING '
      WRITE(8,*) ' AND FINAL INTERPOLATION '
      WRITE(8,888)
...
C
      ...
      THETA   = (N-1)*PI/(NXPTS-1)
      XIN(N)  = 0.5*(1.-COS(THETA))
      IF(XIN(N).LT.0.1)  N4 = N
      IF(XIN(N).LT.0.90) N80 = N
      XARG    = XMIN + XIN(N)*XCHORD
C
...
C
      ...
      YSIN(N) = (YAVG + (YSIN(N)-YAVG)/FTHICK(N))/XCHORD
      YPIN(N) = (YAVG + (YPIN(N)-YAVG)/FTHICK(N))/XCHORD
      TKIN(N) = ABS(YSIN(N)-YPIN(N))
      XIN(N)  = (XARG - XMIN)/XCHORD
      WRITE(8,889) N,XIN(N),YSIN(N),YPIN(N),TKIN(N)
   55 CONTINUE
      XCHORD  = 1.0
      YCHORD  = 0.5*(YPIN(NXPTS) + YSIN(NXPTS))
...
cC*************************************************************************
cC*************************************************************************
cC    IF  HGRAPH IS NOT AVAILABLE DELETE FROM HERE TO STATEMENT 37
cC*************************************************************************
cC*************************************************************************
cC
cC      NOW PLOT THE BLADE SECTION GENERATED AND THE THICKNESS DISTRIBUTION
cC      ON THE SCREEN AND ENQUIRE IF IT IS ACCEPTABLE.
cC
c      WRITE(6,*)'******************************************************'
c      WRITE(6,*)'******************************************************'
cC
...
c      CALL BRKPLT 
cC
cC      PLOT THE THICKNESS DISTRIBUTION
cC  
c      CALL SELPLT(9)
c      CALL GRFAR(XIN,TKIN,NXPTS,1,'o',1,0)
c      CALL XLABEL('X  COORDINATE ',14)
c      CALL YLABEL('TANGENTIAL THICKNESS',20)
c      CALL TITLE(0.25,0.02,0.75,0.02,0,'THICKNESS DISTRIBUTION',22)
c      CALL BRKPLT
cC
cC     PLOT AN ENLARGED VIEW OF THE LEADING EDGE
cC
c      CALL SELPLT(9)
c      CALL GRFAR(XIN,YSIN,N4,1,'+',1,1) 
c      CALL PLTAR(XIN,YPIN,N4,1,'x') 
c      CALL XLABEL('X  COORDINATE ',14)
c      CALL YLABEL('Y  COORDINATE ',14)
c      CALL TITLE(0.05,0.05,0.10,0.05,0,'LEADING EDGE DETAIL',19)
c      CALL BRKPLT 
cC 
cC
cC     PLOT AN ENLARGED VIEW OF THE TRAILING EDGE
cC
c      YLAB = YCHORD - 0.05
c      CALL SELPLT(9)
c      CALL GRFAR(XLTE,YSTE,NPTE,1,'+',0,1)
c      CALL PLTAR(XLTE,YPTE,NPTE,1,'x')
c      CALL XLABEL('X  COORDINATE ',14)
c      CALL YLABEL('Y  COORDINATE ',14)
c      CALL TITLE(0.93,YLAB,0.98,YLAB,0,'TRAILING EDGE DETAIL',20)
c      CALL BRKPLT
cC
c      WRITE(6,36)
c   36 FORMAT(  ' IS THIS OK,  Y or N ?') 
c      READ(5,38) ANS
c   38 FORMAT(A1)
cC
c      WRITE(6,*)'******************************************************'
...
cC
cC
c      IF(ANS.EQ.'Y'.OR.ANS.EQ.'y') GO TO 37 
cC
cC     STOP THE RUN IF THE BLADE IS NOT ACCEPTABLE.
cC
c      WRITE(6,*) ' PROGRAM TERMINATED TO ALLOW THE INPUT DATA'
c      WRITE(6,*) ' FILE   "stagen.dat"   TO BE CHANGED. '
cC
cC
cC     WRITE OUT THE NEW SURFACE SLOPES TO FILE NEWSLOP '
cC
c      OPEN(UNIT=13,FILE='NEWSLOP')
c      DO N = 1,NPIN
c      SUP  = ATAN(SLPUP(N))/DEGRAD
c      SDWN = ATAN(SLPLOW(N))/DEGRAD
c      WRITE(13,550) FRACX(N),SNEWUP(N),SNEWLOW(N),SUP,SDWN
c      END DO
c  550 FORMAT(' FRACTIONAL CHORD ',F10.3,' NEW SURFACE SLOPES ',2F10.3, 
c     &       ' PREVIOUS SLOPES ',2F10.3)
c      CLOSE(13)
cC
c      STOP
cC
