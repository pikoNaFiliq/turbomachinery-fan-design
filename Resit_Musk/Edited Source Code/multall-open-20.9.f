...
C   OF THE  PARAMETER STATEMENTS IN "commall-open-20.9"                C
C                                                                      C
...
C
     ...
     &                    PNOW,TNOW,ENTPY_NOW,GA_PV_NOW,DRYNOW)
           ...
           RONOW   = PO1(K)/(PHRATIO*HO_INN(K))
C           WRITE(6,*) 'K , PHRATIO, RONOW, PNOW,TNOW,HNOW',
C     &                 K,PHRATIO,RONOW,PNOW,TNOW,HNOW
  596 CONTINUE
           WRITE(6,*)
...
C
C      WRITE(6,*)
C      WRITE(6,*) ' CALLING TABSEARCH, STEP = ', NSTEP
C      WRITE(6,*)
      DO 5900 K=1,KM
C      CHNGED J=2 ,Jm  To J=1,Jm FOR STEAM SO TABSEARCH IS CALLED AT J = 1.
      DO 5900 J=1,JM
      DO 5900 I=1,IM
C
           RONOW = RO(I,J,K)
...
C           IF(J.EQ.1.AND.I.EQ.IMID) THEN
C           WRITE(6,*) 'U,RO,P,T,H,RGAS,PHEXP,H_TO_T',
C     &                  UNOW, RONOW, PNOW,TNOW,HNOW,
C     &                  PO_RO/TNOW,PO_RO/HNOW,TNOW/HNOW
C          END IF
      END IF
C
 5900 CONTINUE
...
C     IF THIS IS THE PRESSURE SURFACE.
     ...
     &   - (TK_SET - TE_NOW)*ROVMER(J_TEDGE)*TK_SURF(J_TEDGE)
      END IF
C
C      IF(MOD(NSTEP,500).EQ.0) THEN
C               WRITE(6,*) ' CONDITIONS ON I1 SURFACE , SUM_SRC =',
C     &         SUM_SOURCE
C               WRITE(6,*) '          J      PERR,        SURF_FLUX,      
C     &    TE_FLUX         NET_FLUX           SUM_FLUX ' 
C      END IF
...
C
C      IF(MOD(NSTEP,500).EQ.0)  THEN
C      P_AVG_I1 = 0.25*(P(1,J,1)+P(1,J,2)+P(1,J-1,1)+P(1,J-1,2))
C      PERR     = (PSPEC_I1(J) - P_AVG_I1)/P_AVGG(J)       
C      WRITE(6,*) J, PERR, SURF_FLUX(J),TE_FLUX,TOT_FLUX,SUM_SOURCE_I1(J)
C      END IF
C
...
C
C      IF(MOD(NSTEP,500).EQ.0) THEN
C               WRITE(6,*) ' CONDITIONS ON IM SURFACE , SUM_SRC =',
C     &         SUM_SOURCE
C               WRITE(6,*) '          J      PERR,        SURF_FLUX,      
C     &    TE_FLUX      NET_FLUX        SUM_FLUX ' 
C      END IF
...
C
C      IF(MOD(NSTEP,500).EQ.0)  THEN
C      P_AVG_IM = 0.25*(P(IM,J,1)+P(IM,J,2)+P(IM,J-1,1)+P(IM,J-1,2))
C      PERR     = (PSPEC_IM(J) - P_AVG_IM)/P_AVGG(J)       
C      WRITE(6,*) J, PERR, SURF_FLUX(J),TE_FLUX,TOT_FLUX,SUM_SOURCE_IM(J)
C      END IF
C
C     CHANGE THE SURFACE MASS FLUX IN RESPONSE TO THE PRESSURE DIFFERENCES.
C
...
C
     ...
     &     RFIN1*P(I,1,K) + RFIN2*P(I,1,K)- RFIN*P(I,3,K)
C
...
C
C******************************************************************************
C      CHECK FOR POINTS OUTSIDE THE TABULATED DATA
C
...
C
 ...
 9003 CONTINUE
      WRITE(6,*)'POINTS OUTSIDE TABLULATED INPUT DATA, ROLOW, ROHIHIGH,
     &ULOW ,UHIGH', NRO_LOW, NRO_HI, NU_LOW, NU_HI
...
C
     ...
     &              RATFLOW
 9000 FORMAT(I5,F8.4,3I4,2F8.4,I6,2F8.2,3I4,3F12.4,I10)
C
C******************************************************************************
C******************************************************************************
...
C
      IF(IFGAS.EQ.3) THEN
C    SET THE GAS CONSTANT,RGAS_IN, AT INLET IF IFGAS = 3.
           RONOW = ROSTAG_IN(K)
           UNOW  = HO_INN(K) - PO1(K)/RONOW
           CALL TABSEARCH(RONOW,UNOW,
     &                    PNOW,TNOW,ENTPY_NOW,GA_PV_NOW,DRYNOW)
           RGAS_IN = PNOW/(RONOW*TNOW)
      END IF
C 
...
C
      IF(IFGAS.EQ.3) THEN
             ROS  = ROSTAG_IN(K)*PRAT**(1.0/GA_PV_NOW)            
             TS   = PGUESS(J)/(ROS*RGAS_IN)
      END IF                    
C
...
C    
     ...
     &                    PNOW,TNOW,ENTPY_NOW,GA_PV_NOW,DRYNOW)
      ...
      UNEW   = HO(I,J,K) -EKE - P(I,J,K)/RONOW
C      IF(I.EQ.IMID.AND.K.EQ.KMID) THEN
C      IF(NITS.EQ.1) WRITE(6,*) ' J = ', J
C      WRITE(6,*) 'ITERATION No', NITS,'DENSITY= ',RONOW,'ENERGY= ',UNEW 
C      END IF
 3311 CONTINUE
C 
      RO(I,J,K) = RONOW
...
C 
      END IF
C      
 3322 FORMAT('J= ',I5,' INITIAL GUESS OF: T, P ,RO, VX, VR, VT ',
     &        F10.2, F12.1, 4F10.3)
...
C 
      ROE(I,J,K)    = RO(I,J,K)*HO(I,J,K) - P(I,J,K)
C
...
C
      IF(IFGAS.EQ.3) THEN
           HO_IN  =  SUMHO(J1)
           HO_OUT =  SUMHO(J2M1)
           AVGEXP =  (PHEXP(IMID,J1,KMID) + PHEXP(IMID,J2M1,KMID))/2
           HO_OUT_IS = HO_IN*PORAT**AVGEXP
           HSTAT2_IS = HO_IN*PSRAT**AVGEXP
           DELHO = HO_OUT - HO_IN
           DELHO_IS  = HO_OUT_IS - HO_IN
           DELH_TOT_STAT = HSTAT2_IS - HO_IN
           ETAIS     = 1.0
           IF(DELHO.GT.0.0) ETAIS = DELHO_IS/DELHO
           IF(DELHO.LT.0.0) ETAIS = DELHO/DELHO_IS
           ETATS   = 1.0
...
C
C      CHECK FOR ANY LARGE VARIATIONS BETWEEN ADJACNT POINTS
C
      DO 94  I = 1,ITAB
      DO 94, J = 2,JTAB-1
      IF(ABS(VAR(I,J)-VAR(I,J-1))/ABS(VAR(I,J-1)).GT.0.1)THEN
      VAR(I,J) = 0.5*(VAR(I,J-1) + VAR(I,J+1))
